...
// Build content array maintaining exact PDF order
function buildExactPDFStructure() {
  const allContent = [];
  ...
  pages.forEach((pageText, pageIdx) => {
    ...
    // Process content maintaining order
    // Split by sentences but preserve structure
    const parts = [];
    
    // Handle special cases for images based on page number
    const imageTriggers = {
      1: [
    ...
    sentences.forEach((sentence, sentIdx) => {
      ...
      const triggers = imageTriggers[pageNum] || [];
...
const content = buildExactPDFStructure();
...
// Update first section with PDF content
