...
export function generateMetadata({ params }: PageProps): Metadata {
  const pageData = menuPagesData.pages.find(
    page => page.category === 'resources' && page.slug === params.slug
  );

  if (!pageData) {
    return {
      title: 'Resources | OHG365',
      description: 'Access free programming resources, cheat sheets, and study materials.',
    };
  }
    ...
    return (
      <main className="min-h-screen py-20" style={{ backgroundColor: 'var(--bg-primary)' }}>
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-gray-400">The resource page you're looking for doesn't exist.</p>
        </div>
      </main>
    );
  }
