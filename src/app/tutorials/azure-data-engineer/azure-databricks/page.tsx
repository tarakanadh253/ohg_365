...
import courseSections from '@/data/azure-databricks-course.json';
...
const SectionContent = ({ content }: { content: CourseSection['content'] }) => {
  ...
  content.forEach((item, index) => {
    if (item.type === 'image') {
      pendingImages.push({
        src: item.src,
        alt: item.alt || 'Azure Databricks image',
        width: 1920,
        height: 1080
      });
      return;
...
export default function AzureDatabricksPage() {
  const [activeSection, setActiveSection] = useState(ALL_SUBSECTIONS[0]?.id || 'azure-databricks');
  ...
  useEffect(() => {
    const handleHashChange = () => {
      ...
      if (!hash) {
        setActiveSection(ALL_SUBSECTIONS[0]?.id || 'azure-databricks');
        setActiveSubsection(null);
        return;
      }
      ...
      <div className="min-h-screen relative px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12 sm:mb-16 relative z-10">
          ...
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-3 sm:mb-4">
            Azure Databricks <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-red-500">Module</span>
          </h1>
          <p className="text-gray-400 text-base sm:text-lg md:text-xl max-w-3xl mx-auto">
            Master Azure Databricks for data engineering, analytics, and machine learning
          </p>
        </div>
        ...
        {currentContent && (
          ...
          >
            <div className="flex items-center gap-2 text-sm text-gray-400 mb-4">
              ...
              <span>›</span>
              <span className="text-orange-400">{currentContent.section.title}</span>
            </div>
